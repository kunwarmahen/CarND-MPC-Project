 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      ...
      fg[2 + y_start + t] = y1 - (yt + v0 * CppAD::sin(psi0) * dt);
      fg[2 + psi_start + t] = psi1 - (psi0 - (v0 * delta * (dt) / Lf));
      fg[2 + v_start + t] = v1 - (v0 + (a * (dt)));
      fg[2 + cte_start + t] = cte1 - (f0 - yt + (v0 * CppAD::sin(epsi0) * dt));
      fg[2 + epsi_start + t] = epsi1 - (psi0 - psi_des - (v0 * delta * (dt) / Lf));
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  response.push_back(solution.x[delta_start]);
  response.push_back(solution.x[a_start]);

  for (unsigned int i = 0; i<10; i++) {
	  response.push_back(solution.x[x_start + i]);
	  response.push_back(solution.x[y_start + i]);
  }
  return response;
}
